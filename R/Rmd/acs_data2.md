...
diploma_df %>% filter(type == "all") %>% write_csv("../output/acs_no_high_school_diploma_by_location.csv")
diploma_df %>% filter(name == "New Haven") %>% write_csv("../output/acs_no_high_school_diploma_by_race.csv")
```
